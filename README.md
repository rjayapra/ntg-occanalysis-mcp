...
An Azure Storage Emulator is needed for this particular sample because we will read occupation data from blob storage.
...
1. In Copilot chat agent mode enter a prompt to trigger the tool, e.g., enter these prompts:
   ...
   ```plaintext
   Get job codes for rank S1 in NavComm
   ```

   ```plaintext
   Get all duty areas for occupation NavComm
   ```
...
| Tool           | Description                                        |
| -------------- | -------------------------------------------------- |
| `hello_mcp`    | A simple hello world tool                          |
| `get_snippet`  | Retrieve a snippet by name from Azure Blob Storage |
| `save_snippet` | Save a snippet with a name to Azure Blob Storage   |

### Occupation Data Tools

These tools query occupation data stored as JSON files in Azure Blob Storage:

| Tool                         | Description                             | Parameters                                                                  |
| ---------------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| `list_all_occupations`       | List all available occupations          | None                                                                        |
| `get_available_occupations`  | Get occupation data (all or specific)   | `occupation` (optional)                                                     |
| `get_all_ranks`              | Get all unique ranks in occupation data | `occupation` (optional)                                                     |
| `get_job_codes_for_rank`     | Get job codes for a specific rank       | `rank` (required), `occupation` (optional)                                  |
| `get_tasks_for_job_code`     | Get tasks for a specific job code       | `job_code` (required), `occupation` (optional)                              |
| `get_tasks_for_duty_area`    | Get tasks for a specific duty area      | `duty_area_code` (required), `job_code` (optional), `occupation` (optional) |
| `get_skills_for_job_code`    | Get skills for a specific job code      | `job_code` (required), `occupation` (optional)                              |
| `get_knowledge_for_job_code` | Get knowledge items for a job code      | `job_code` (required), `occupation` (optional)                              |
| `get_all_duty_areas`         | Get all unique duty areas               | `occupation` (optional)                                                     |

### Example Prompts for Occupation Tools
...
```plaintext
Get all ranks for NavComm occupation
```
...
```plaintext
Get tasks for job code 514157 in NavComm
```

```plaintext
Get all duty areas for NavComm
```

The function code is defined in the Python files in the `src` directory. The MCP function annotations expose these functions as MCP Server tools.
